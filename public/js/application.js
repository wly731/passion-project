...
var backtestSubmit = function(){
	$( '#backtest_info' ).on( 'submit', function(e){
		...
		for (var j=0;j<num_of_stocks;j++) {
			...
			var stock_query_string = `${Object.keys(stock_allocation)[j]}.json?`;
			var api_key = `api_key=Aqaga5u9KDtditK1Nua9`
			var option = `&order=desc&start_date=${start_date}&end_date=${end_date}&column_index=11&transformation=rdiff`

			

			var complete_url = api + stock_query_string + api_key + option;
			...
			request.done(function(json){
				...
				// var count = json["query"]["count"];
				// var num_of_stocks_and_index = num_of_stocks + 1
				var num_of_days = json.dataset.data.length; // + 1 because need to include the index (NASDAQ)
				...
				for (var i=0;i<num_of_days-1;i++){ //iterate all days e.g. 0 to 63 //row for the table
					...
					var today_closing = json.dataset.data[i][1];
					var yesterday_closing = json.dataset.data[i-1][1];
					var proportion = stock_allocation[Object.keys(stock_allocation)[j]];
					var proportional_daily_return = Number((((today_closing/yesterday_closing) - 1)*proportion*100).toPrecision(4));


					performance_table[date][Object.keys(stock_allocation)[0]] = proportional_daily_return;
						$ ( `#${date}` ).append(`<td>${proportional_daily_return}</td>`);
